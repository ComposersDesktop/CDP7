...
int main(int argc,char *argv[])
{
    ...
    if(!sloom) {
        ...
        if(dz->mode == 0 && cmdlinecnt < 8) {
            ...
            usage2("newtex");
            return(FAILED);
        } else if(dz->mode == 2 && cmdlinecnt < 11) {
            usage2("newtex");
            return(FAILED);
        }
...
int newtex(int *perm,int *permon,int *permoff,int *superperm, double minrate,int maxsteps,dataptr dz)
{
    ...
    double loval, hival, valdiff, timefrac, val = 0.0, valr = 0.0, vall = 0.0, maxval = 1.0, pos = 0.0;
    float *obuf, **ibuf;
    ...
    double *tabptr = NULL;
    double **llev = NULL, **rlev = NULL, **transval = NULL;
