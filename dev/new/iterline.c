...
#define oflowsize    temp_sampsize
...
static int read_the_input_snd(double *maxinsamp,int passno,dataptr dz);
static int iter_shift_interp_stereo(int cnt,int passno, double *gain,double *pshift,int local_write_start,
		int inmsampsize,double *maxoutsamp,int pstep,int iterating,double thistrans,dataptr dz);
...
{
	if(!strcmp(prog_identifier_from_cmdline,"iterline"))				dz->process = ITERLINE;
	else {
...
int usage2(char *str)
{
	if(!strcmp(str,"iterline")) {
		fprintf(stdout,
	    "USAGE:\n"
	    "iterline iterline mode infile outfile tdata outduration\n"
	...
	while(fgets(temp,200,fp)!=NULL) {
		...
		while(get_float_from_within_string(&p,&dummy)) {
			if(istime) {
				...
				lasttime = dummy;
			} else if(dummy > 24 || dummy < -24) {
				sprintf(errstr,"Found transposition value (%lf) out of range (-24 to +24) in file %s\n",dummy,str);
				return(DATA_ERROR);
			}
			istime = !istime;
			cnt++;
...
int do_iterate_preprocess(dataptr dz)
{
	...
	int mindelay_samps, inmsampsize, maxoverlay_cnt;
...
int create_the_iterbufs(double maxpscat,dataptr dz)
{
	...
	overflowsize = dz->insams[0] * 4;
	overflowsize += ITER_SAFETY; 						/* 2 */	
	if((seccnt = overflowsize/F_SECSIZE) * F_SECSIZE < overflowsize)
		seccnt++;
	overflowsize = F_SECSIZE * overflowsize;
...
int iterline(dataptr dz)
{
	...
	for(passno=0;passno<2;passno++) {
		...
		if((exit_status = read_the_input_snd(&maxinsamp,passno,dz))<0)
			return(exit_status);
		if(maxinsamp <= 0.0) {
		...
		if(dz->iparam[ITER_DO_SCALE]) {
			for(n=0; n < dz->infilespace; n++)
				dz->sampbuf[0][n] = (float)(dz->sampbuf[0][n] * dz->param[ITER_GAIN]);
		}
		inmsampsize = dz->infilespace/dz->infile->channels;	/* no. of 'stereo'-samps to process */
		/* 1 */
		local_write_start = 0;
		switch(dz->iparam[ITER_PROCESS]) {
		case(ST_INTP_SHIFT):      	
			write_end = iter_shift_interp_stereo(0,passno,gain,pshift,local_write_start,inmsampsize,&maxoutsamp,pstep,iterating,thistrans,dz);		
			break;
		case(FIXA_ST_INTP_SHIFT): 	
			write_end = fixa_iter_shift_interp_stereo(0,passno,gain,pshift,local_write_start,inmsampsize,&maxoutsamp,pstep,iterating,thistrans,dz); 
			break;
		}
		thistime = 0.0;
		...
		for(;;) {
			...
			if(dz->brksize[ITER_DELAY])
				dz->iparam[ITER_MSAMPDEL] = round(dz->param[ITER_DELAY] * (double)dz->infile->srate);
			write_end = iterate(cnt,passno,gain,pshift,write_end,local_write_start,inmsampsize,&maxoutsamp,pstep,iterating,thistrans,dz);
			if(passno==0)
				wstart[cnt] = get_next_writestart(write_start,dz);
			write_start = wstart[cnt];
...
/* TEST */
fprintf(stdout,"INFO: thistrans = %lf dz->param[pstep] = %lf\n",thistrans,dz->param[pstep]);
fprintf(stdout,"INFO: inmsampsize = %d insamps = %d\n",inmsampsize,dz->insams[0]);
fflush(stdout);
/* TEST */
	if(passno == 0) {
...
/* TEST */
if(i%256 == 0) {
fprintf(stdout,"INFO: i = %d d = %lf j = %d buflen = %d + overflow = %d\n",i,d,j,dz->buflen,dz->oflowsize);
fflush(stdout);
}
/* TEST */
...
int read_the_input_snd(double *maxinsamp,int passno,dataptr dz)
{
	...
	if(passno == 0) {
		for(k=0;k < dz->insams[0];k++) {
			if(fabs(dz->sampbuf[0][k]) > *maxinsamp)
				*maxinsamp = fabs(dz->sampbuf[0][k]);
		}
	}
	return(FINISHED);
}
 ...
 int inmsampsize,double *maxoutsamp,int pstep,int iterating,double thistrans,dataptr dz)
{
	...
	switch(dz->iparam[ITER_PROCESS]) {
	case(ST_INTP_SHIFT):      	
		wr_end = iter_shift_interp_stereo(cnt,pass,gain,pshift,local_write_start,inmsampsize,maxoutsamp,pstep,iterating,thistrans,dz);		
		break;
	case(FIXA_ST_INTP_SHIFT): 	
		wr_end = fixa_iter_shift_interp_stereo(cnt,pass,gain,pshift,local_write_start,inmsampsize,maxoutsamp,pstep,iterating,thistrans,dz); 
		break;
	}
