...
int find_exact_peaktimes(int envcnt,int *peaks,dataptr dz)
{
    int exit_status;
    double maxsamp, val;
    int n, t, maxtime, samptime, searchend, ibufstart_in_file, start, end;
    float *env = dz->env;
    float *buf = dz->sampbuf[0];
    if((exit_status = read_samps(buf,dz))<0)
        return(exit_status);
...
int find_exact_troftimes(int trofcnt,int *trofs,dataptr dz)
{
    ...
    int n, t, p, searchstt, searchend, start, end, minpeakat, troftime;
    int searchlen;
...
int shrink2(dataptr dz)
{
    ...
    int splen = dz->iparam[SHR_SPLEN], dblsplenlen, randrange = 0, offset;
...
int find_outsamps(dataptr dz)
{
    ...
    int splen = dz->iparam[SHR_SPLEN], dblsplenlen;
