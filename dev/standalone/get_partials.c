...
static int	check_getpartials_param_validity_and_consistency(dataptr dz);
...
static int  setup_getpartials_application(dataptr dz);
static int  setup_getpartials_param_ranges_and_defaults(dataptr dz);
...
static int partials(dataptr dz);
static int extract_harmonic_contour(dataptr dz);
...
/************************* SETUP_GETPARTIALS_PARAM_RANGES_AND_DEFAULTS *******************/

int setup_getpartials_param_ranges_and_defaults(dataptr dz)
{
	int exit_status;
	aplptr ap = dz->application;
	// set_param_ranges()
	...
	ap->lo[0]			= SPEC_MINFRQ;
	ap->hi[0]			= dz->nyquist;
	ap->default_val[0]	= CONCERT_A;
	ap->lo[1]			= FLTERR;
	ap->hi[1]			= 1;
	ap->default_val[1]	= 0.01;
	dz->maxmode = 4;
	if(dz->mode > 1) {
		ap->lo[2]			= 0.0;
		ap->hi[2]			= dz->duration;
		ap->default_val[2]	= dz->duration/2.0;
	}
	if(!sloom)
...
{
	if      (!strcmp(prog_identifier_from_cmdline,"harmonic"))			dz->process = PARTIALS_HARM;
	else {
...
int usage2(char *str)
{
	if(!strcmp(str,"harmonic")) {
		fprintf(stderr,
		"USAGE: get_partials harmonic 1-2 inanalfile outfile fundamental threshold [-v]\n"
		"OR:    get_partials harmonic 3-4 inanalfile outfile fundamental threshold time [-v]\n"
		"\n"
		"Extract relative amplitudes of partials in a pitched source.\n"
		"\n"
		"MODE 1: Convert single analysis window to FRQ and amplitude list\n"
		"MODE 2: Convert single analysis window to MIDI and amplitude list\n"
		"MODE 3: Convert window at time TIME to FRQ and amplitude list\n"
		"MODE 4: Convert window at time TIME to MIDI and amplitude list\n"
		"\n"
		"INANALFILE    For Modes 1-2, file must have only a single analysis window.\n"
		"FUNDAMENTAL   Fundamental frequency of sound.\n"
		"THRESHOLD     (relative) level below which partials are ignored.\n"
		"TIME          (multiple window files only) Time of window to use.\n" 
		"-v    Gives output in format for filter varibank2 file\n"
		"\n");
	} else
...
int partials(dataptr dz)
{
	int exit_status,foundit = 0;
	int wc, samps_read, got, windows_in_buf;
	float wtime=0.0f;
	if(dz->mode > 1)
		wtime = (float)dz->param[2];
	...
	while(!foundit) {
   		for(wc=0; wc<windows_in_buf; wc++) {
			if (dz->total_windows == dz->wlength - 1) {
				foundit = 1;
				break;
			}
			if(dz->time < wtime || dz->total_windows == 0) {
		...
		if(!foundit) {
			if((samps_read = fgetfbufEx(dz->bigfbuf, dz->buflen,dz->ifd[0],0)) < 0) {
		 	...
		 	dz->flbufptr[0] = dz->bigfbuf;
			if(samps_read <= 0) {
				sprintf(errstr,"Failed to find window.\n");
				return(PROGRAM_ERROR);
			}
		}
	}
	switch(dz->process) {
	case(PARTIALS_HARM):
		if((exit_status = extract_harmonic_contour(dz))<0)
			return(exit_status);
	}
	return FINISHED;
}

/*********************** CHECK_GETPARTIALS_PARAM_VALIDITY_AND_CONSISTENCY ***************************/

int	check_getpartials_param_validity_and_consistency(dataptr dz)
{
	if(dz->mode <= 1) {
		if(dz->wlength != 1) {
			sprintf(errstr,"This mode only works with single-window analysis files\n");
			return(DATA_ERROR);
		}
	}
	return(FINISHED);
...
int extract_harmonic_contour(dataptr dz)
{
	int harmno, n, cc, vc, try_again, all_finished, valid_amps;
	double partialfrq, maxamp, frqratio, fundamental = dz->param[0];
	float *partial_amp, frq, amp;
	dz->clength	= dz->wanted / 2;
	if((partial_amp = (float *)malloc(dz->clength * sizeof(float)))==NULL) {
		sprintf(errstr,"no memory for partial amplitudes store\n");
		return(MEMORY_ERROR);
	}
	harmno = 0;
	partialfrq = fundamental * (harmno+1);
	partial_amp[harmno] = 0.0;
	all_finished = 0;
		...
		while(try_again) {
			...
			if(frq <= partialfrq) {
				...
				if(frqratio < EIGHT_OVER_SEVEN)					/* if approx equal to harmonic frq */
					partial_amp[harmno] = 
					(float)max((double)amp,partial_amp[harmno]);/* store max amp val found for this harmonic */
	...
	maxamp = 0.0;
	for(n = 0;n<=harmno;n++)
		maxamp = max((double)partial_amp[n],maxamp);			/* Find loudest partial */
	if(maxamp <= FLTERR) {
	...
	valid_amps = 0;
	for(n = 0;n<=harmno;n++) {
		partial_amp[n] = (float)(partial_amp[n]/maxamp);		/* Normalise the spectral contour */
		if(partial_amp[n] > dz->param[1])
			valid_amps = 1;
	}
	if(!valid_amps) {
		sprintf(errstr,"No significant harmonics found\n");
		return(GOAL_FAILED);
	}
	if(dz->vflag[0]) {											/* in varipartial case, store pitch data */
		if(dz->mode == 1 || dz->mode == 3)
			frq = (float)unchecked_hztomidi(dz->param[0]);
		else
			frq = (float)dz->param[0];
		fprintf(dz->fp,"0\t%f\t1.0\n",frq);
		fprintf(dz->fp,"100000\t%f\t1.0\n",frq);
		fprintf(dz->fp,"#\n");
	}
	if(dz->vflag[0])
		fprintf(dz->fp,"0\t");
	for(n = 0;n<=harmno;n++) {									/* store frq and amp of significant partials */
		frq = (float)(fundamental * (n+1));
		if(partial_amp[n] > dz->param[1]) {
			if(dz->vflag[0])
				fprintf(dz->fp,"%d  %f  ",n+1,partial_amp[n]);
			else {
				if(dz->mode == 1 || dz->mode == 3)
					frq = (float)unchecked_hztomidi(frq);
				fprintf(dz->fp,"%f  %f\n",frq,partial_amp[n]);
			}
		}
	}
