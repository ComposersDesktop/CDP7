...
static int  	value_greater_than_brkvals(double val,double *brk,long brksize,int repel);
static int  	value_less_than_brkvals(double val,double *brk,long brksize,int repel);
static int 		brk_compare(double *brk1,long brksize1,double *brk2,long brksize2,int repel);
...
int getpeakdata(int ifd, float *peakval,long *peakpos,SFPROPS props);
...
void validate(int applicno,long *valid);
...
int float_array(float **q,long size)
{
...
int brk_compare(double *brk1,long brksize1,double *brk2,long brksize2,int repel)
{
	long cnt1 = 0, cnt2 = 0;
...
int value_greater_than_brkvals(double val,double *brk,long brksize,int repel)
{
	double *p = brk;
...
int value_less_than_brkvals(double val,double *brk,long brksize,int repel)
{
	double *p = brk;
	double *pend = p + (brksize*2);
	p++;
...
int establish_additional_brktable(dataptr dz)
{
	...
	if(dz->extrabrkno > 0) {
		...
		|| (dz->brksize  = (long    *)realloc(dz->brksize  ,brkcnt * sizeof(long)    ))==NULL
		...
		|| (dz->brksize  = (long    *)malloc(brkcnt * sizeof(long)    ))==NULL
...
int initialise_param_values(int process,int mode,int channels,double nyquist,float frametime,
	long insams,long srate,long wanted,int linecnt,double duration,double *default_val,int filetype,aplptr ap)
{
	int    n;
...
(int process,int mode,int total_params,double nyquist,float frametime,float arate,long srate,
long wlength,long insams,int channels,long wanted,
...
(int process,int mode,double nyquist,float frametime,float arate,long srate,
long wlength,long insams,int channels,long wanted,
int filetype,int linecnt,double duration,aplptr ap)
{
 	int    n;
...
int readhead(infileptr inputfile,int ifd,char *filename,double *maxamp,double *maxloc, long *maxrep,int getmax,int needmaxinfo)
{
	float peakval;
	long peakpos;
	SFPROPS props = {0};
...
//TW TEMPORARY SUBSTITUTION
	...
	else if(sndgetprop(ifd,"original sampsize",(char *) &os,sizeof(long)) > 0)
...
*/
	...
	case(SNDFILE):
		if(needmaxinfo) {
			*maxrep = (long)-1;	/* flags that maxinfo not got, unless it's got, below */
			*maxloc = (long)0;	
			*maxamp = 0.0;
			if(getmax) {
...
// TW SUBSTITUTE FOR NOW
		if(sndgetprop(ifd,"orig channels",(char *)&(inputfile->origchans),sizeof(long)) < 0) {
...
// TW SUBSTITUTE FOR NOW
		if(sndgetprop(ifd,"original sampsize",(char *)&(inputfile->origstype),sizeof(long)) < 0) {
			fprintf(stdout,"Cannot read original sample type: %s\n",filename);
			fflush(stdout);
			return(DATA_ERROR);
		}
		if(sndgetprop(ifd,"original sample rate",(char *)&(inputfile->origrate),sizeof(long)) < 0) {
...
{
	long pppp = 1;
    if(sndgetprop(fd,"is a pitch file", (char *)&pppp, sizeof(long)) < 0)
		return(FALSE);
    return(TRUE);
}   
...
{
	long tttt = 1;
    if(sndgetprop(fd,"is a transpos file", (char *)&tttt, sizeof(long)) < 0)
		return(FALSE);
    return(TRUE);
}   
...
{	
	long ffff = 1;
    if(sndgetprop(fd,"is a formant file", (char *)&ffff, sizeof(long)) < 0)
		return(FALSE);
    return(TRUE);
}   
...
int headread_check(SFPROPS props,char *filename,int ifd)
{
	...
	if(sndgetprop(ifd,"sample rate", (char *)&(inputfile->srate), sizeof(long)) < 0){
	...
	if(sndgetprop(ifd,"channels", (char *)&(inputfile->channels), sizeof(long)) < 0){
	...
	if(sndgetprop(ifd,"is an envelope",(char *)&dummy,sizeof(int)) >= 0)
		...
		inputfile->filetype = FORMANTFILE;
	else if(sndgetprop(ifd,"original sampsize", (char *)&(inputfile->origstype), sizeof(long)) >= 0)
		inputfile->filetype = ANALFILE;
	...
	case(FORMANTFILE):
		if(sndgetprop(ifd,"specenvcnt", (char *)&(inputfile->specenvcnt), sizeof(long)) < 0){
...
{
	CHPEAK peaks[16];
	long chans, peaktime, maxpos = -1, pos_set = 0;
	float maxamp = -2.0f;
	...
	if(sndreadpeaks(ifd,chans,peaks,&peaktime)){
		...
		for(i=0;i < chans;i++) {
			if(peaks[i].value > maxamp) {
				...
				if(pos_set && (peaks[i].position < (unsigned long)maxpos))
					maxpos = peaks[i].position;
			}
		}
		if(maxpos < 0) {
