...
//TW NEW CASE
        ...
        case(ENV_CREATE):
          ...
          if(mode==ENV_ENVFILE_OUT
             || mode==ENV_BRKFILE_OUT)       ap->special_data = ENV_CREATEFILE;
          break;
        case(ENV_WARPING):
        case(ENV_REPLOTTING):
        case(ENV_RESHAPING):
                if(mode==ENV_TRIGGERING)     ap->special_data = ENV_TRIGGER_RAMP;
                break;
        case(MIXINBETWEEN):
                if(mode==INBI_RATIO)
                ...
                break;
        case(MIXSHUFL):
                if(mode==MSH_DUPL_AND_RENAME) ap->special_data=SNDFILENAME;
                break;
