...
int param_preprocess(dataptr dz)	
{
	...
	case(RRRR_EXTEND):
		if(dz->mode != 1) {
			...
			dz->iparam[RRR_AT]    = (int)round(dz->param[RRR_AT] * dz->infile->srate) * dz->infile->channels;
			dz->iparam[RRR_AFTER] = (int)round(dz->param[RRR_AFTER] * dz->infile->srate) * dz->infile->channels;
			dz->iparam[RRR_TEMPO] = (int)round(dz->param[RRR_TEMPO] * dz->infile->srate) * dz->infile->channels;
		}
...
int check_param_validity_and_consistency(dataptr dz)
{
	...
	case(RRRR_EXTEND):
		...
		if(dz->mode == 1) {
			dz->param[RRR_WSIZE] = LOW_RRR_SIZE/3.0;	/* 15 ms/2 = 5ms */
...
int create_rrr_sndbufs(dataptr dz)
{
	...
	if(dz->mode == 1) {
		secfactor = max((int)(dz->iparam[RRR_SAMP_WSIZE]),F_SECSIZE) * 2;
...
int generate_samp_windowsizer(dataptr dz)
{
	...
	dz->iparam[RRR_SAMP_WSIZE] = (int)(SECSIZE/sizeof(float));
	unadjusted_envwindow_sampsize = round(dz->param[RRR_WSIZE] * MS_TO_SECS * (double)srate);
	/* FOLLOWING POSSIBLY UNNECESSARY WITH NEW WINSIZE MINIMUM */
	if(unadjusted_envwindow_sampsize < dz->iparam[RRR_SAMP_WSIZE]) {
		k = dz->iparam[RRR_SAMP_WSIZE];
		while(unadjusted_envwindow_sampsize<k)
			k /= 2;
		j = k * 2;
		if(j - unadjusted_envwindow_sampsize > k - unadjusted_envwindow_sampsize)
			dz->iparam[RRR_SAMP_WSIZE] = (int)k;
		else
			dz->iparam[RRR_SAMP_WSIZE] = (int)j;
	}
	if(unadjusted_envwindow_sampsize > dz->iparam[RRR_SAMP_WSIZE]) {
		k = round((double)unadjusted_envwindow_sampsize/(double)dz->iparam[RRR_SAMP_WSIZE]);
		dz->iparam[RRR_SAMP_WSIZE] = (int)(dz->iparam[RRR_SAMP_WSIZE] * k);
	}
	return(FINISHED);
}
