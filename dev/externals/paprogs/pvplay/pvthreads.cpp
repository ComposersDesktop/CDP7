...
__inline void stereo_interls(const float *in_l,const float *in_r,float *out,int insize)
{
    int i;
                ...
                if (file_playing)  {
                    ...
                    for(i = 0;i < framestoget;i++){ 
                        ...
                        if(got && (framepos <= pdata->to_frame)) {
                            int samps = pdata->pv_l->process_frame(pdata->analframe1,pbuf,PVOC_AMP_FREQ);
                            elementstowrite += samps;
                            pbuf += samps;
                        }
                ...
                if(file_playing){
                    ...
                    for(i = 0;i < framestoget;i++){
                        ...
                        if(got){
                            ...
                            if(got == -1) {
                                fprintf(stderr,"\nError reading ch 2 from pvx file, frame %u\n",framepos);
                                file_playing  = 0;
                                break;
                            }
                        }
