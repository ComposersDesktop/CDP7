...
int do_convolve(dataptr dz)
{
	...
	int n = 0, m = 0, dl, yy, ynext, nn, mm, samps_read, maxwrite;
	...
	do {
		for(k=0;k<chans;k++) {
			...
			case(CONV_NORMAL):
				...
				break;
			case(CONV_TVAR):
				for(n = k, thistime = 0.0; n < dz->ssampsread; n+=chans, thistime += timeincr) {
					nn = n - k;
...
//TW Omission corrected DEC 2002
		...
		if(samps_read >= dz->buflen) {
			...
			if(maxwrite > 0) {
				...
				if((exit_status = write_samps(dz->sampbuf[2],maxwrite,dz))<0)
					return(exit_status);
				display_virtual_time(dz->total_samps_written,dz);
			}
			break;
